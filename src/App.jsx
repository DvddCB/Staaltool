...
async function fetchProfileRole(userId, accessToken) {
  ...
  return normalizeRoleValue(profile?.role || "worker");
}
...
export default function App() {
  ...
  useEffect(() => {
    async function syncRoleFromSupabaseSession() {
      try {
        ...
        const role = await fetchProfileRole(session.user.id, session.access_token);
        ...
        console.error("Rol synchroniseren mislukt:", err);
      }
    }

    syncRoleFromSupabaseSession();
  }, []);
  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      const role = await fetchProfileRole(userId, accessToken);
        ...
        <header style={styles.header} className="app-header-responsive">
          <div style={styles.brandRow} className="app-brand-responsive">
            ...
            <div>
              ...
              <p style={styles.headerSubtitle}>Artikelcodes voor circulaire bouwmaterialen · {isAdmin ? "Beheerder" : isReception ? "Receptie" : "Gebruiker"} · Rol: {currentRole || "-"}</p>
            </div>
          </div>
