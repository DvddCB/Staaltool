  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      if (!profileResponse.ok) {
        ...
        console.error("Profile fout:", msg);
        setError("Profiel fout: " + msg);
        return;
      }
      ...
      const role = normalizeRoleValue(profile?.role || "worker");

      localStorage.setItem("staaltoolLoggedIn", "true");
      localStorage.setItem("staaltoolUserRole", String(role || "").trim().toLowerCase());
        ...
        <header style={styles.header} className="app-header-responsive">
          <div style={styles.brandRow} className="app-brand-responsive">
            ...
            <div>
              ...
              <p style={styles.headerSubtitle}>Artikelcodes voor circulaire bouwmaterialen · {isAdmin ? "Beheerder" : String(userRole || "").trim().toLowerCase() === "receptie" ? "Receptie" : "Gebruiker"}</p>
            </div>
          </div>
          ...
          <section style={styles.pickerHomePage} className="picker-home-responsive">
            <section style={styles.panel}>
              <div style={styles.pickerPanelHeader} className="pickerPanelHeader">
                <div>
                  ...
                  {!isAdmin && (
                    <p style={styles.workerNotice}>{isAdmin
                  ? "Je werkt als beheerder. Uploaden, datums wijzigen en verwijderen zijn ingeschakeld."
                  : isReception
                  ? "Je werkt als receptie. Je kunt alleen pakbon en stickerbon openen."
                  : "Je werkt als gebruiker. Uploaden, datums wijzigen en verwijderen zijn uitgeschakeld."}</p>
                  )}
                </div>
            ...
            <section style={styles.agendaPanel}>
              ...
              <section style={styles.selectedOrderPanel}>
                ...
                {(isAdmin || isReception) && (selectedPickerOrder?.pakbon_url || selectedPickerOrder?.stickerbon_url) && (
                  <div style={styles.pdfButtonRow}>
                    {selectedPickerOrder?.pakbon_url && (
                      <button
                        style={styles.pdfButton}
                        onClick={() => window.open(selectedPickerOrder.pakbon_url, "_blank")}
                      >
                        Pakbon openen
                      </button>
                    )}
                    {selectedPickerOrder?.stickerbon_url && (
                      <button
                        style={styles.pdfButtonAlt}
                        onClick={() => window.open(selectedPickerOrder.stickerbon_url, "_blank")}
                      >
                        Stickerbon openen
                      </button>
                    )}
