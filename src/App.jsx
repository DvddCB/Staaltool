...
function extractMemoFromRawPdfText(rawText) {
  const text = String(rawText || "").replace(/\s+/g, " ").trim();
  const match = text.match(/Medewerker:\s*[^ ]+\s+(.*?)\s+Memo\s+Pickbon/i);
...
function orderFromDb(row) {
  ...
  return {
    ...
    memo: row.memo || extractMemoFromRawPdfText(row.raw_pdf_text || row.rawPdfText || "")
  };
}
  ...
  useEffect(() => {
    async function handleRecoveryUrl() {
      ...
      try {
        ...
        // Supabase stuurt tegenwoordig vaak ?code=... terug.
        // Die code moet eerst omgewisseld worden voor een tijdelijke sessie,
        // daarna kan updateUser({ password }) werken.
        if (code) {
          const { error: exchangeError } = await supabase.auth.exchangeCodeForSession(code);

          if (exchangeError) {
            console.error("Recovery code exchange fout:", exchangeError);
            setError("Herstel-link kon niet worden geopend: " + (exchangeError.message || "Onbekende fout"));
            return;
          }
        ...
        console.error(err);
        setError("Herstel-link fout: " + (err.message || String(err)));
      }
    }
...
function isOrderOpen(order) {
  return String(order.status || "").toLowerCase() !== "gereed";
}
...
function getOrderProgress(order) {
  ...
  if (String(order?.status || "").toLowerCase() === "gereed") {
...
async function fetchRoleFromSupabaseProfile(userId, accessToken) {
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(userId)}&select=role,name`,
    {
      headers: {
        apikey: SUPABASE_ANON_KEY,
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(message || "Profiel kon niet worden geladen");
  }
  ...
  return {
    ...
    role: normalizeRoleValue(profile?.role || "worker")
  };
}
...
export default function App() {
  ...
  const [userRole, setUserRole] = useState(() => normalizeRoleValue(localStorage.getItem("staaltoolUserRole") || ""));
  ...
  useEffect(() => {
    async function refreshRoleFromActiveSession() {
      try {
        ...
        const { rawProfile, role } = await fetchRoleFromSupabaseProfile(
          session.user.id,
          session.access_token
        );

        localStorage.setItem("staaltoolLoggedIn", "true");
        localStorage.setItem("staaltoolUserRole", role);
        localStorage.setItem("staaltoolUserEmail", session.user.email || "");

        setUserRole(role);
        setLoggedIn(true);
        setRoleDebug(
          `email=${session.user.email || "-"} | uid=${session.user.id} | profileRole=${rawProfile?.role || "-"} | appRole=${role}`
        );
      } catch (err) {
        console.error("Role refresh fout:", err);
        setRoleDebug("Role refresh fout: " + (err.message || String(err)));
      }
    }

    refreshRoleFromActiveSession();
  ...
  const [showFinishPrompt, setShowFinishPrompt] = useState(true);
  ...
  const currentRole = normalizeRoleValue(userRole || localStorage.getItem("staaltoolUserRole"));
  const isAdmin = currentRole === "admin";
  const isReception = currentRole === "receptie";
  ...
  const effectivePickerOrders = useMemo(() => {
    return uploadedPdfOrders.map((order) => ({
      ...order,
      status: processedOrderIds.includes(order.id) || String(order.status || "").toLowerCase() === "gereed" ? "Gereed" : order.status
    }));
  ...
  async function loadOrdersFromSupabase() {
    ...
    try {
      ...
      setProcessedOrderIds(orders.filter((order) => String(order.status || "").toLowerCase() === "gereed").map((order) => order.id));
  ...
  async function handleUpdatePassword(event) {
    ...
    try {
      ...
      if (updateError) {
        setError("Wachtwoord wijzigen mislukt: " + (updateError.message || "Onbekende fout"));
        return;
      }
      ...
      setResetMessage("Wachtwoord aangepast. Log opnieuw in met je nieuwe wachtwoord.");

      window.history.replaceState({}, document.title, window.location.pathname);
    } catch (err) {
      console.error(err);
      setError("Wachtwoord wijzigen mislukt: " + (err.message || String(err)));
    }
  }
  ...
  async function handlePasswordReset() {
    ...
    try {
      ...
      if (resetError) {
        setError("Wachtwoord herstellen mislukt: " + (resetError.message || "Onbekende fout"));
        return;
      }

      setResetMessage("Herstelmail verstuurd. Controleer je inbox.");
    } catch (err) {
      console.error(err);
      setError("Wachtwoord herstellen mislukt: " + (err.message || String(err)));
    } finally {
  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      if (authError) {
        ...
        setError("Login fout: " + (authError.message || "Onbekende Supabase fout") + " | project: " + SUPABASE_URL);
        return;
      }
      ...
      const { rawProfile, role } = await fetchRoleFromSupabaseProfile(userId, accessToken);

      setRoleDebug(
        `email=${email} | uid=${userId} | profileRole=${rawProfile?.role || "-"} | appRole=${role}`
      );

      if (!rawProfile) {
        setError("Login gelukt, maar geen profiel/rol gevonden voor UID: " + userId + ". Voeg deze UID toe in Supabase profiles.");
        return;
      }

      localStorage.setItem("staaltoolLoggedIn", "true");
      localStorage.setItem("staaltoolUserRole", normalizeRoleValue(role));

      setUserRole(normalizeRoleValue(role));
      setLoggedIn(true);
      setError("");
    } catch (err) {
      console.error(err);
      setError("Login crash: " + (err.message || String(err)) + " | project: " + SUPABASE_URL);
    }
  }
  ...
  function logout() {
    ...
    localStorage.removeItem("staaltoolUserEmail");
    setUserRole("");
    setRoleDebug("");
    setLoggedIn(false);
    setSelectedModule("");
  ...
  function confirmProcessLine() {
    ...
    setPickbonLines((currentLines) => {
      ...
      persistCurrentOrderRows(nextLines, "Open");

      if (allDone && currentSelectedPickerOrder?.id) {
        // Alle regels zijn gepickt, maar de order wordt nog NIET op Gereed gezet.
        // Dat gebeurt pas als gebruiker in de popup zelf op Verwerken klikt.
        updateOrderInSupabase(currentSelectedPickerOrder.id, {
          status: "Open",
          rows: nextLines.map((line) => ({
            articleCode: line.articleCode || line.artikelcode || "",
            artikelcode: line.articleCode || line.artikelcode || "",
            artikel: getDisplayArticleDescription(line),
            description: getDisplayArticleDescription(line),
            ...
            colorName: line.colorName || "",
            quantity: Number(line.originalQuantity || line.quantity || 1),
            originalQuantity: Number(line.originalQuantity || line.quantity || 1),
            aantal: Number(line.originalQuantity || line.quantity || 1),
            scannedQuantity: Number(line.originalQuantity || line.quantity || 1),
            processed: true,
            status: "gepikt",
            scannedAt: line.scannedAt || new Date().toLocaleString("nl-NL")
          })),
          regels: nextLines.length
        }).catch((err) => {
          console.error(err);
          setSearchError("Gepickte regels konden niet direct worden opgeslagen in Supabase.");
        });
      }
  ...
  function handleFinishPickbonFromPopup() {
    if (!currentSelectedPickerOrder?.id) return;
    ...
    const finishedLines = pickbonLines.map((line) => {
      ...
      return {
        ...
        scannedQuantity: requestedQuantity,
        processed: true,
        scannedAt: new Date().toLocaleString("nl-NL")
      };
    });
    ...
    updateOrderInSupabase(currentSelectedPickerOrder.id, {
      ...
      regels: finishedLines.length
    }).then(() => {
      loadOrdersFromSupabase();
    }).catch((err) => {
      console.error(err);
      setSearchError("Pickbon is lokaal verwerkt, maar kon niet direct naar Supabase worden opgeslagen.");
    });
    ...
    setUploadedPdfOrders((currentOrders) =>
      currentOrders.map((order) =>
        order.id === currentSelectedPickerOrder.id ? { ...order, status: "Gereed", rows: finishedLines } : order
      )
    );
  ...
  function confirmOrderActionNow() {
    ...
    if (confirmOrderAction === "finish") {
      ...
      updateOrderInSupabase(selectedPickerOrder.id, {
        status: "Gereed",
        rows: finishedLines.map((line) => ({
          articleCode: line.articleCode || line.artikelcode || "",
          artikelcode: line.articleCode || line.artikelcode || "",
        ...
        console.error(err);
        setSearchError("Order is lokaal verwerkt, maar kon niet direct naar Supabase worden opgeslagen.");
      });
  ...
  function openPickerOrder(order) {
    ...
    if (effectiveOrder?.rows?.length) {
      ...
      if (String(effectiveOrder.status || "").toLowerCase() === "gereed") {
        window.setTimeout(() => {
          setPickbonLines((currentLines) =>
            currentLines.map((line) => ({
              ...line,
  ...
  if (passwordRecoveryMode) {
    return (
      <div style={styles.loginPage}>
        <form style={styles.loginCard} onSubmit={handleUpdatePassword}>
          <img src="/logo.png" alt="logo" style={styles.loginLogo} />
          <h2 style={styles.loginTitle}>NIEUW WACHTWOORD</h2>
          ...
          <button style={styles.loginButton} type="submit">Wachtwoord opslaan</button>
    ...
    return (
      <div style={styles.loginPage}>
        <form style={styles.loginCard} onSubmit={handleLogin}>
          ...
          <input style={styles.loginInput} placeholder="E-mailadres" value={username} onChange={(e) => setUsername(e.target.value)} />
          ...
          >
            {resetLoading ? "Mail versturen..." : "Wachtwoord vergeten?"}
          </button>
          {resetMessage && <p style={styles.resetMessage}>{resetMessage}</p>}
          {error && <p style={styles.loginError}>{error}</p>}
        </form>
    ...
    <div style={styles.appPage}>
      ...
      <div style={styles.appShell}>
        <header style={styles.header} className="app-header-responsive">
          <div style={styles.brandRow} className="app-brand-responsive">
            ...
            <div>
              ...
              <p style={styles.headerSubtitle}>Artikelcodes voor circulaire bouwmaterialen · {isAdmin ? "Beheerder" : isReception ? "Receptie" : "Gebruiker"}</p>
              <p style={{ ...styles.headerSubtitle, color: "#dc2626", fontWeight: 800 }}>Rol debug: {roleDebug || currentRole || "-"}</p>
            </div>
          </div>
        ...
        {confirmOrderAction && (
          <div style={styles.confirmOverlay}>
            <div style={styles.confirmModal}>
              <h2 style={styles.confirmTitle}>{confirmOrderAction === "finish" ? "Pickbon verwerken" : "Order aanpassen"}</h2>
              ...
              <div style={styles.confirmActions}>
                ...
                <button style={styles.confirmYesButton} onClick={confirmOrderActionNow}>{confirmOrderAction === "finish" ? "Verwerken" : "Ja"}</button>
              </div>
            </div>
          </div>
        )}
                  ...
                  {pagedPickerOrders.map((order) => (
                    <button
                      ...
                      style={
                        selectedPickerOrder?.id === order.id
                          ? String(order.status || "").toLowerCase() === "gereed"
                    ...
                    >
                      <div style={styles.orderCardTop} className="orderCardTop">
                        ...
                        <span style={String(order.status || "").toLowerCase() === "gereed" ? styles.doneStatus : styles.todoStatus}>
                          {String(order.status || "").toLowerCase() === "gereed" ? "✓ Verwerkt" : "Nog doen"}
                        </span>
                      </div>
                        ...
                        {dayOrders.map((order) => (
                          <button
                            ...
                            style={{
                              ...
                              background: String(order.status || "").toLowerCase() === "gereed" ? "#e2e8f0" : "#eab308",
                              color: String(order.status || "").toLowerCase() === "gereed" ? "#166534" : "#0f172a"
                            }}
                            onClick={() => openPickerOrder(order)}
                          >
                            <span style={styles.calendarOrderTime}>{order.klant || order.tijd}</span>
                            <strong>{String(order.status || "").toLowerCase() === "gereed" ? "✓ " : ""}{order.id}</strong>
                          </button>
                        ))}
                      </div>
                    );
              ...
              <div style={styles.pickbonPanel}>
                <div style={styles.pickbonHeader}>
                  <div>
                    ...
                    <div style={styles.pickbonTitleRow}>
                      <h2 style={styles.pickbonOrderTitle}>{pickbonNumber || "Nieuwe pickbon"}</h2>
                      {currentSelectedPickerOrder?.memo && (
                        <span style={styles.memoBadge}>Let op memo</span>
                      )}
                    </div>
                  ...
                  <div style={styles.pickbonActions}>
                    {showFinishPrompt && allPickbonLinesProcessed && String(currentSelectedPickerOrder?.status || "").toLowerCase() !== "gereed" ? (
                      <div style={styles.inlineProcessBox}>
                        <strong style={styles.inlineProcessTitle}>Pickbon verwerken</strong>
                        <div style={styles.inlineProcessButtons}>
                          <button style={styles.confirmYesButton} onClick={handleFinishPickbonFromPopup}>Verwerken</button>
                        </div>
                      </div>
                    ) : (
                ...
                {String(currentSelectedPickerOrder?.status || "").toLowerCase() === "gereed" && (
                    ...
                    {pickbonLines.map((line) => (
                      <div key={line.id} style={line.processed ? styles.pickbonLineDone : styles.pickbonLine} className="pickbon-line-responsive">
                        ...
                        <div style={styles.pickbonLineControls} className="pickbon-controls-responsive">
                          ...
                          {!line.processed && Number(line.scannedQuantity || 0) < Number(line.originalQuantity || line.quantity || 1) && (
                            <button style={styles.smallDarkButton} onClick={() => requestProcessLine(line.id)}>Artikel picken</button>
                          )}
                ...
                {pickerView === "pickbon" && currentSelectedPickerOrder?.memo && (
                  <div style={styles.memoPanel}>
                    <div style={styles.memoPanelTitle}>Memo / maatwerk</div>
                    <div style={styles.memoPanelText}>{currentSelectedPickerOrder.memo}</div>
                  </div>
                )}
              </div>
            </section>
